...
function dashboard2() {
    ...
    function dashboard() {
      ...
      $enquiry=$this->admin_model->getAllEnquiryCount($start_date,$end_date);
       if(intval($total_fess)>0){
		$data['fees_overview']=array(
            'total_unpaid'=>(intval($total_fess)-intval($total_paid))-intval($partial),
            'unpaid_progress'=>(((intval($total_fess)-intval($total_paid))-intval($partial))*100)/intval($total_fess),
            'total_partial'=>$partial,'total_paid'=>$total_paid,
            'partial_progress'=>(intval($partial)*100)/intval($total_fess),
            'total_paid'=>$total_paid,
            'paid_progress'=>(intval($total_paid)*100)/intval($total_fess),
        );
       }else{
		$data['fees_overview']=array(
            'total_unpaid'=>0,
        ...
        if($total_fess>0){
            $data['fessprogressbar']=(intval($total_paid)*100)/intval($total_fess);
        }else{
