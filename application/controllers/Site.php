           ...
           $data['name'] = $app_name;
            ...
            if ($result) {
                if ($result->is_active) {
                    ...
                    $role = "";
                    foreach($result->roles as $key=>$r){
                        $role = $key;
                    }

                    if($role == "Super Admin"){
                        $requisition_count = $this->common_model->dbSelect("count(*) as count","requisitions"," approval_count < 5 AND deleted_at IS NULL ");
                    } else {
                        $requisition_count = $this->common_model->dbSelect("count(*) as count","requisitions"," staff_id = $result->id AND approval_count < 5 AND deleted_at IS NULL ");
                    }
                    if(count($requisition_count) > 0){
                        $requisition_count = $requisition_count[0]->count;
                    } else {
                        $requisition_count = 0;
                    }
